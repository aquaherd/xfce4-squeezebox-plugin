...
G_DEFINE_TYPE_WITH_PRIVATE(GMpd, g_mpd, G_TYPE_OBJECT)
...
static gboolean _error_extract(GMpdPrivate *priv, const gchar* line)
{
   ...
   {
      _error_clear(priv);
      priv->error = g_error_new(G_MPD_DOMAIN, G_MPD_ERROR, "GMpd-Error:%s",
            line);
      return TRUE;
   }
   return FALSE;
...
{
   GOutputStream *ostm = g_io_stream_get_output_stream(
...
static void _idle_cancel(GMpdPrivate *priv)
{
   if (priv->inIdle)
   {
      ...
      do
      {
         ...
         isOK = NULL != line
               && (!g_strcmp0(line, "OK") || g_str_has_prefix(line, "ACK "));
         g_free(line);
      } while (!isOK);
   }
}
...
{
   GMpdPrivate *priv = g_mpd_get_instance_private(user_data);
   GSocket *socket = g_socket_connection_get_socket(priv->connection);
   LOG("EnterThread");
   while (g_socket_condition_wait(socket,
         G_IO_IN | G_IO_PRI | G_IO_ERR | G_IO_HUP, NULL, NULL))
   {
      if (priv->inIdle)
      {
   ...
   if (_send_command_raw(priv, "listplaylists\n"))
   {
      ...
      while (line && g_strcmp0(line, "OK"))
      {
         if (g_str_has_prefix(line, "playlist: "))
         {
            ...
            g_hash_table_insert(priv->playlists, newList, g_strdup("playlist"));
         }
         g_free(line);
         line = _read_response_line(priv);
      }
...
static void _update_status(GMpdPrivate *priv, gchar *changeDetail)
{
   ...
   if (_send_command_raw(priv, "status\n"))
   {
      ...
      while (line && g_strcmp0(line, "OK"))
      {
         ...
         changed |= g_strcmp0(newValue,
               g_hash_table_lookup(priv->stateinfo, oldKey));
...
static void _update_currentsong(GMpdPrivate *priv, gchar *changeDetail)
{
   if (_send_command_raw(priv, "currentsong\n"))
   {
      ...
      while (line && !g_str_has_prefix(line, "OK"))
      {
         ...
         gchar *newValue = g_strdup(tokens[1] + 1);
         changed |= g_strcmp0(newValue,
               g_hash_table_lookup(priv->currentsong, oldKey));
...
static void g_mpd_dispose(GObject *gobject)
{
   ...
   GMpdPrivate *priv = g_mpd_get_instance_private(G_MPD(gobject));
   LOG("Dispose");
      ...
      const GValue *value, GParamSpec *pspec)
{
   ...
   priv = g_mpd_get_instance_private(G_MPD(self));
   switch (property_id)
   {
      /*
       case PROP_HOST:
...
static void g_mpd_get_property(GObject *self, guint property_id, GValue *value,
      GParamSpec *pspec)
{
   GMpdPrivate *priv;
   g_return_if_fail(G_IS_MPD (self));
...
static void g_mpd_class_init(GMpdClass *klass)
{
   ...
   klass->signal[SIGNAL_IDLE] = g_signal_new("idle-changed",
         ...
         0 /* offset */,
         NULL /* accumulator */,
         NULL /* accumulator data */, g_cclosure_marshal_VOID__VOID,
         G_TYPE_NONE /* return_type */, 1 /* n_params */,
   ...
   klass->signal[SIGNAL_STATUS] = g_signal_new("status-changed",
         ...
         NULL /* accumulator */,
         NULL /* accumulator data */, g_cclosure_marshal_VOID__VOID,
         G_TYPE_NONE /* return_type */, 0 /* no params */
         );
   ...
   /* properties */
   pspec = g_param_spec_string("host", "MPD Host", "Set MPD's host",
         "localhost" /* default value */, G_PARAM_READABLE);
   g_object_class_install_property(gobject_class, PROP_PASS, pspec);

   pspec = g_param_spec_int("port", "MPD Port", "Set MPD's port", 1, 65535,
         6600 /* default value */, G_PARAM_READABLE);
   g_object_class_install_property(gobject_class, PROP_PASS, pspec);

   pspec = g_param_spec_string("password", "MPD Passphrase",
         "Set MPD's passphrase", NULL /* default value */,
         G_PARAM_READWRITE);
   g_object_class_install_property(gobject_class, PROP_PASS, pspec);
...
static void g_mpd_init(GMpd *self)
{
   GMpdPrivate *priv;
   G_MPD_DOMAIN = g_quark_from_static_string("MPD");
   self->priv = priv = g_mpd_get_instance_private(self);
   priv->currentsong = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
         g_free);
   priv->playlists = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
         g_free);
   priv->stateinfo = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
         g_free);
   priv->changes = g_hash_table_new_full(g_str_hash, g_str_equal, g_free,
         _free);
...
void g_mpd_disconnect(GMpd *self)
{
   g_return_if_fail(G_IS_MPD (self));
   {
      GMpdPrivate *priv = g_mpd_get_instance_private(self);
...
gboolean g_mpd_next(GMpd *self)
{
   GMpdPrivate *priv;
   g_return_val_if_fail(G_IS_MPD (self), FALSE);
   priv = g_mpd_get_instance_private(self);
